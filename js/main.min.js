...
const filesInput = document.getElementById("filesInput"),
    clearFiles = (filesInput.addEventListener("change", e => {
...
async function readProviderData() {
    try {
        var e, t, a, s = await fetch("http://186.233.187.210/api/getProviderInfo", {
                referrerPolicy: "http://186.233.187.210/api/getProviderInfo"
            }),
            i = await s.json();
        console.log("provider info:"), console.log(i), s.ok ? (e = new TonWeb.utils.Address(i.result.address).toString(!0, !0, !1), a = await (t = await fetch("http://186.233.187.210/api/getProviderParams", {
            referrerPolicy: "http://186.233.187.210/api/getProviderParams"
        })).json(), console.log("provider params:"), console.log(a), t.ok ? ($("#providerAddress").innerHTML = `<a href="https://ton.cx/address/${e}" target="_blank" class="link">${e}</a>`, $("#providerStatus").innerHTML = Number(a.result.accept_new_contracts) ? '<div class="status-true">True</div>' : '<div class="status-false">False</div>', $("#providerRateSvg").style.display = "flex", $("#providerRate").innerText = Number(a.result.rate_per_mb_day) / 1e9, $("#providerMaxSpan").innerText = Number(a.result.max_span) + " sec", $("#providerMinFileSize").innerText = Number(a.result.minimal_file_size) + " bytes", $("#providerMaxFileSize").innerText = Number(a.result.maximal_file_size) + " bytes") : await errorMessage(a.message)) : await errorMessage(i.message)
    } catch (e) {
...
const tonweb = new TonWeb(new TonWeb.HttpProvider("https://toncenter.com/api/v2/jsonRPC")),
    ...
    onTonReady = async () => {
        if (console.log("tonready"), !window.tonProtocolVersion || window.tonProtocolVersion < 1) alert("Please, update your TON Wallet Extension");
            ...
            const d = window.ton;
            console.log("isTonWallet=", d.isTonWallet), d.on("accountsChanged", function (e) {
                setAccountAddress(e[0])
            });
            var e;
            e = (await d.send("ton_requestAccounts"))[0], console.log(e), setAccountAddress(e), await !console.log(await d.send("ton_requestWallets")), $("#createContractButton").addEventListener("click", async () => {
                try {
                    ...
                    if (0 < t.length) {
                        ...
                        var e = await fetch("http://186.233.187.210/api/upload", {
                                method: "POST",
                                body: a
                            }),
                            s = await e.json();
                    ...
                    var i, n, r, o, l = $("#providerAddress").innerText,
                        c = $("#bagidInput").value;
                    BAGID_REGEXP.test(c) ? ($("#uploadFilesTimeline").classList.remove("timeline__item--current"), $("#sendRequestTimeline").classList.add("timeline__item--active"), $("#sendRequestTimeline").classList.add("timeline__item--current"), i = ((new Date).getTime() / 1e3).toFixed(0), r = await (n = await fetch("http://186.233.187.210/api/newContractMessage", {
                        method: "POST",
                        headers: {
                        ...
                        body: JSON.stringify({
                            provider: new tonweb.utils.Address(l).toString(!1, !1, !1),
                            torrent: c,
                            queryId: i
                        })
                    })).json(), console.log("contract message:"), console.log(r), n.ok ? (o = $("#initInput").value, TOPUP_REGEXP.test(o) ? (d.send("ton_sendTransaction", [{
                        to: l,
                        value: 1e9 * $("#initInput").value + 58067e3,
                        data: r.result.payload,
                        dataType: "boc"
                    }]), $("#sendRequestTimeline").classList.remove("timeline__item--current"), $("#registerTimeline").classList.add("timeline__item--active"), $("#registerTimeline").classList.add("timeline__item--current"), async function (e) {
                        ...
                        for (; !a;) {
                            ...
                            if (s.length) {
                                try {
                                    ...
                                    n == OP_CONTRACT_REGISTERED && e == r && ($(".msg").style.display = "flex", $("#newContractAddress").innerHTML = `<a href="https://ton.cx/address/${s[0].in_msg.source}" target="_blank" class="link">${s[0].in_msg.source}</a>`, a = !0)
                                } catch (e) {
                        ...
                        $("#registerTimeline").classList.remove("timeline__item--current"), $("#waitingConfTimeline").classList.add("timeline__item--active"), $("#waitingConfTimeline").classList.add("timeline__item--current"), async function () {
                            let e = $("#newContractAddress a").innerText,
                                t = !1;
                            await new Promise(e => setTimeout(e, 1100));
                            for (; !t;) - 1 == (await tonweb.provider.call2(e, "get_storage_contract_data", []))[0].toNumber() && (t = !0, $("#waitingConfTimeline").classList.remove("timeline__item--current"), $("#lastTimeline").classList.add("timeline__item--active")), await new Promise(e => setTimeout(e, 1100))
                        }()
                    }(i)) : (resetTimeline(), await errorMessage("Please, enter the valid initial balance"))) : (resetTimeline(), await errorMessage(r.message))) : (resetTimeline(), await errorMessage("Please, enter the valid BAG ID"))
                } catch (e) {
                ...
                if (ADDRESS_REGEXP.test(e) || NON_BOUNCEABLE_ADDRESS_REGEXP.test(e) || BOUNCEABLE_ADDRESS_REGEXP.test(e)) try {
                    var t = await tonweb.provider.call2(e, "get_storage_contract_data", []),
                        a = ($("#contractStatus").innerHTML = t[0].toNumber() ? '<div class="status-true">True</div>' : '<div class="status-false">False</div>', $("#contractBalance").innerText = t[1] / 1e9, parseTextAddress(t[2])),
                        s = (console.log("provider: " + a), $("#contractProvider").innerHTML = `<a href="https://ton.cx/address/${a}" target="_blank" class="link">${a}</a>`, $("#contractFileSize").innerText = t[4], $("#contractRate").innerText = t[6] / 1e9, parseTextAddress(t[9]));
                    console.log("owner: " + s), $("#contractOwner").innerHTML = `<a href="https://ton.cx/address/${s}" target="_blank" class="link">${s}</a>`, $("#contractData").style.display = "flex", s === $("#accountAddress").innerText && ($(".block__aside").style.display = "flex")
                } catch (e) {
                ...
                if (ADDRESS_REGEXP.test(e) || NON_BOUNCEABLE_ADDRESS_REGEXP.test(e) || BOUNCEABLE_ADDRESS_REGEXP.test(e)) {
                    ...
                    if (console.log(t), TOPUP_REGEXP.test(t)) {
                        try {
                            ...
                            a.bits.writeUint(s, 64), d.send("ton_sendTransaction", [{
                                to: e,
                                value: 1e9 * t,
                                data: tonweb.utils.bytesToBase64(await a.toBoc()),
                                dataType: "boc"
                            }])
                        } catch (e) {
                ...
                if (ADDRESS_REGEXP.test(e) || NON_BOUNCEABLE_ADDRESS_REGEXP.test(e) || BOUNCEABLE_ADDRESS_REGEXP.test(e)) {
                    ...
                    try {
                        ...
                        t.bits.writeUint(a, 64), d.send("ton_sendTransaction", [{
                            to: e,
                            value: "30000000",
                            data: tonweb.utils.bytesToBase64(await t.toBoc()),
                            dataType: "boc"
                        }])
                    } catch (e) {
